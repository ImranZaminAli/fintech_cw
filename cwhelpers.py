...
def part_b(n, market_args, r_list, algos, initial_seed):
    ...
    for i in range(len(r_list)):
        ...
        col = i % 3 
        ax = axes[row, col]
        sns.violinplot(data=profits, inner='box', cut=0, ax=ax)
        ax.set_title(f'Trader Ratio: {": ".join(str(r) for r in r_vals)}', fontsize=16)
...
def part_c(n, market_args, r_vals, algos, initial_seed):
    ...
    for i in range(len(r_vals)):
        ...
        if len(set(r_vals)) == 1: # no need to permute if all elements are equal
            ax = sns.violinplot(data=profits, inner='box', cut = 0)
            ax.set_title(f'Trader Ratio: {": ".join(str(r) for r in r_vals)}', fontsize=16)
            ax.set_ylabel("Profits", fontsize = 14)
            return stat_summary
        else:
